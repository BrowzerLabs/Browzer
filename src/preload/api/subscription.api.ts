...
export const createSubscriptionAPI = (): SubscriptionAPI => ({
  ...
  // Credit Management
  useCredits: (creditsToUse: number) =>
    invoke('subscription:use-credits', creditsToUse),
  hasCredits: (creditsNeeded: number) =>
    invoke('subscription:has-credits', creditsNeeded),
