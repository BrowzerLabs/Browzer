...
export class AutocompleteHandler extends BaseHandler {
  register(): void {
    ...
    this.handle('autocomplete:get-suggestions', async (_, query: string): Promise<AutocompleteSuggestion[]> => {
      return browserService.getAutocompleteSuggestions(query);
    });

    this.handle('autocomplete:get-search-suggestions', async (_, query: string): Promise<string[]> => {
