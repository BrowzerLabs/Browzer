    ...
    switch (toolName) {
      ...
      case 'create_tab': {
        ...
        if (!createResult.success) {
          return {
            success: false,
            content: createResult.error ?? 'Failed to create tab',
          };
        }
        const createdTabId = createResult.value;
        return {
          success: true,
          content: `Tab created successfully. Use tabId: "${createdTabId}" for subsequent actions on this tab.`,
        };
      }
