...
export class PasswordManager {
  ...
  constructor() {
    ...
    console.log(
      '[PasswordManager] Initialized with store at:',
      this.store.path
    );
  }
  ...
  private deriveEncryptionKey(): Buffer {
    try {
      ...
      console.error(
        '[PasswordManager] Failed to derive encryption key:',
        error
      );
      // Fallback: generate random key (will lose passwords but prevents crashes)
      console.warn(
        '[PasswordManager] Using fallback random key - passwords may be lost'
      );
  ...
  public async saveCredential(
    ...
    try {
      ...
      console.log(
        `[PasswordManager] Saved credential for ${username} at ${origin}`
      );
      return true;
    } catch (error) {
  ...
  public deleteMultipleCredentials(credentialIds: string[]): boolean {
    try {
      const credentials = this.store.get('credentials');
      const filteredCredentials = credentials.filter(
        (c) => !credentialIds.includes(c.id)
      );

      this.store.set('credentials', filteredCredentials);
      console.log(
        `[PasswordManager] Deleted ${credentials.length - filteredCredentials.length} credentials`
      );
      return true;
    } catch (error) {
  ...
  public async importPasswords(
    ...
    try {
      ...
      if (parsed.credentials && Array.isArray(parsed.credentials)) {
        for (const cred of parsed.credentials) {
          try {
            ...
            errors++;
            console.error(
              '[PasswordManager] Failed to import credential:',
              error
            );
          }
        }
      ...
      if (parsed.blacklist && Array.isArray(parsed.blacklist)) {
        ...
        const newBlacklist = [
          ...new Set([...currentBlacklist, ...parsed.blacklist]),
        ];
