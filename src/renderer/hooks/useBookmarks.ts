...
import type { Bookmark, BookmarkTreeNode, CreateBookmarkParams } from '@/shared/types';
...
interface UseBookmarksReturn {
  ...
  toggleBookmark: (url: string, title: string, favicon?: string) => Promise<{ added: boolean; bookmark: Bookmark | null }>;
...
export function useBookmarks(): UseBookmarksReturn {
   ...
   * Add a new bookmark
   */
  const addBookmark = useCallback(async (params: CreateBookmarkParams): Promise<Bookmark | null> => {
    try {
   ...
   * Update a bookmark
   */
  const updateBookmark = useCallback(async (id: string, title?: string, url?: string): Promise<boolean> => {
    try {
      const result = await window.browserAPI.updateBookmark({ id, title, url });
      if (result) {
        await refreshBookmarks();
      }
      return result;
   ...
   */
  const getBookmarkForUrl = useCallback(async (url: string): Promise<Bookmark | null> => {
    try {
