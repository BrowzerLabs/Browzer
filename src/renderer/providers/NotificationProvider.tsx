...
export function NotificationProvider({ children }: NotificationProviderProps) {
          ...
          action: {
            ...
            onClick: () => {
              ...
              if (handler) {
                ...
                console.warn(
                  `[NotificationProvider] No handler registered for action: ${action.action}`
                );
              }
            },
          },
  ...
  const handleDialogNotification = useCallback(
  ...
  const handleNavigationNotification = useCallback(
    (notification: NotificationPayload) => {
  ...
  const handleCallbackNotification = useCallback(
    (notification: NotificationPayload) => {
      ...
      if (callback_name) {
        ...
        if (handler) {
          ...
          console.warn(
            `[NotificationProvider] No handler registered for callback: ${callback_name}`
          );
    ...
    (notification: NotificationPayload) => {
      try {
        ...
        switch (type) {
          ...
          default:
            console.warn(
              '[NotificationProvider] Unknown notification type:',
              type
            );
        ...
        console.error(
          '[NotificationProvider] Error handling notification:',
          error
        );
  ...
  useEffect(() => {
    unsubscribeRef.current =
      window.notificationAPI.onNotification(handleNotification);
    registerAppNotificationCallbacks();

    const unsubscribeToast = window.notificationAPI.onToast(
      handleToastNotification2
    );
