...
export function TabBar({ 
  ...
  const handleCreateGroupSubmit = useCallback(async () => {
    ...
    try {
      ...
      console.error('[TabBar] Failed to create group:', error);
    } finally {
      handleGroupDialogOpenChange(false);
    }
  }, [newGroupColor, newGroupName, onAssignGroup, onCreateGroup, pendingGroupTabId, handleGroupDialogOpenChange]);
...
function SortableTab({ tab, isActive, isDragging, onClick, onClose, width, tabGroups, onAssignGroup, onCreateGroupRequested, onMenuOpenChange, onRemoveGroup, onDeleteGroup, onToggleCollapse, isGroupStart, isGroupCollapsed, isAnyDragging }: SortableTabProps) {
  const {
    attributes,
    listeners,
    setNodeRef,
