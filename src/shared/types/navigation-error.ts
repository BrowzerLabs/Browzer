...
type ErrorInfo = Omit<
  NavigationError,
  'code' | 'url' | 'timestamp' | 'technicalDetails'
>;
...
const ERROR_INFO: Record<number, ErrorInfo> = {
  ...
  [-201]: {
    ...
    description:
      "The server's security certificate has expired or is not yet valid.",
    isRecoverable: false,
    suggestions: [
  ...
  [-113]: {
    ...
    isRecoverable: false,
    suggestions: ['The website might be using outdated security protocols'],
  },
  [-300]: {
  ...
  [-301]: {
    ...
    isRecoverable: false,
    suggestions: ['Try using http:// or https://'],
  },
  [-302]: {
    category: NavigationErrorCategory.HTTP,
  ...
  [-312]: {
    ...
    isRecoverable: false,
    suggestions: ['Try using a standard port (80 for HTTP, 443 for HTTPS)'],
  },
  [-20]: {
    category: NavigationErrorCategory.BLOCKED,
    ...
    isRecoverable: false,
    suggestions: ['Check if an extension is blocking this request'],
  },
  [-22]: {
    category: NavigationErrorCategory.BLOCKED,
    ...
    isRecoverable: false,
    suggestions: ['Contact your network administrator'],
  },
  [-6]: {
    category: NavigationErrorCategory.HTTP,
  ...
  [-10]: {
    ...
    isRecoverable: false,
    suggestions: ['You might need to log in'],
  },
  [-2]: {
...
const NETWORK_ERROR_CODES = new Set([
  -106, -105, -102, -118, -101, -104, -100, -21, -109, -7,
]);
...
export const shouldIgnoreError = (code: number) =>
  code === NavigationErrorCode.ERR_ABORTED;
